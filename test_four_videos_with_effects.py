...
def create_draft_with_four_videos_and_effects():
    # 服务器地址
    ...
    video_urls = [
        ...
        "http://192.168.8.107:9000/youtube-videos/users/1/projects/1/slices/613143f4-c598-44f8-8627-4750eb1c9599/sub_12b89262-9f2a-4b30-9c8c-317f5443e50a_20250809_072409.mp4",
        "http://192.168.8.107:9000/youtube-videos/users/1/projects/1/slices/7c86fd50-5869-49b3-b088-3d0babc0ab60/sub_a762e477-179c-4336-b887-00668b8833a3_20250809_072410.mp4"
    ]
    
    # 草稿文件夹地址 (Windows路径)
    draft_folder = "E:\\Users\\Administrator\\AppData\\Local\\JianyingPro\\User Data\\Projects\\com.lveditor.draft"
    
    print("=== CapCutAPI 智能测试程序 (四个视频 + 特效) ===")
    ...
    try:
        ...
        for i, (video_url, video_info) in enumerate(zip(video_urls, video_info_list)):
            # 添加Ripple特效 (持续3秒)
            print(f"{i*2+3}. 正在添加第{i+1}个视频的Ripple特效...")
            effect_response = requests.post(f"{BASE_URL}/add_effect", json={
                "draft_id": draft_id,
                "effect_type": "水波纹",  # Ripple特效
            ...
            if not effect_result["success"]:
                ...
                print(f"✓ 第{i+1}个视频的Ripple特效添加成功")
            
            # 添加视频
            print(f"{i*2+4}. 正在添加第{i+1}个视频...")
        ...
        # 4. 添加覆盖文本 (覆盖整个视频时长)
        print(f"{len(video_urls)*2+3}. 正在添加覆盖文本...")
        ...
        # 5. 添加字幕
        print(f"{len(video_urls)*2+4}. 正在添加字幕...")
        ...
        # 6. 保存草稿并上传到MinIO
        print(f"{len(video_urls)*2+5}. 正在保存草稿并上传到MinIO...")
        ...
        for i, video_info in enumerate(video_info_list):
            ...
            print(f"  {start_time:.2f}-{end_time:.2f}秒: 第{i+1}个视频 (开头有Ripple特效)")
            current_time = end_time
        print(f"  0-{total_duration:.2f}秒: 文本显示 (覆盖整个视频)")
        print(f"  0-{total_duration:.2f}秒: 字幕显示 (根据SRT文件时间)")
...
if __name__ == "__main__":
    create_draft_with_four_videos_and_effects()
