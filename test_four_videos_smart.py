...
def create_draft_with_four_videos():
    # 服务器地址
    ...
    try:
        ...
        for i, (video_url, video_info) in enumerate(zip(video_urls, video_info_list)):
            # 添加Ripple特效 (持续3秒)
            print(f"{i+3}. 正在添加第{i+1}个视频的Ripple特效...")
            effect_response = requests.post(f"{BASE_URL}/add_effect", json={
                "draft_id": draft_id,
                "effect_type": "Ripple",  # Ripple特效
